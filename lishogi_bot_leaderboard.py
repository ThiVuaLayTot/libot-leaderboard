...
import sys
import lishogi.api
from lishogi.format import SINGLE_PGN
import json
import os
import datetime
import re
...
def get_file_name(type):
    return './lishogi/' + type + '.md'

def get_all_bots_ratings():
    all_bots_ratings = []
    with open('lishogi_bots.txt', 'r') as f:
        lishogi_bots = [player.strip() for player in f.readlines()]

    batch_size = 100
    num_batches = (len(lishogi_bots) + batch_size - 1) // batch_size

    for i in range(num_batches):
        batch_start = i * batch_size
        batch_end = (i + 1) * batch_size
        batch_usernames = lishogi_bots[batch_start:batch_end]

        users_list = lishogi.api.users_by_ids(batch_usernames)
    ...
    with open('lishogi_bot_leaderboard.json', 'w') as f:
        json.dump(all_bots_ratings, f)
    print("Updated lishogi_bot_leaderboard.json file.")

def get_bots_leaderboard(type):

    file_path = os.path.join(os.path.dirname(__file__), 'lishogi_bot_leaderboard.json')
